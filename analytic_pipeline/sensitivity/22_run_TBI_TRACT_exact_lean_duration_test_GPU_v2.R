# =============================================================================
# 22_run_TBI_TRACT_exact_lean_duration_test_GPU_v2.R
#
# PURPOSE
#   Compare CURRENT_REFERENCE vs an exact LEAN_CLINICAL predictor set for
...
out_dir <- file.path(
  output_dir,
  "TBI_TRACT_EXACT_LEAN_DURATION_TEST"
)
...
if (!"supplemental_oxygen_recovered" %in% lean_clinical) {
  stop(
    "LEAN_CLINICAL unexpectedly removed supplemental oxygen.",
    call. = FALSE
  )
}
...
fwrite(
  ...
  file.path(
    out_dir,
    "03_EXACT_LEAN_DURATION_METRICS.csv"
  )
)
...
fwrite(
  ...
  file.path(
    out_dir,
    "04_EXACT_LEAN_DURATION_RUN_QC.csv"
  )
)
...
fwrite(
  ...
  file.path(
    out_dir,
    "05_EXACT_LEAN_DURATION_DELTAS_VS_CURRENT.csv"
  )
)

summary <- delta[
  variant_id == "LEAN_CLINICAL",
  .(
    n_folds = .N,
...
fwrite(
  ...
  file.path(
    out_dir,
    "06_EXACT_LEAN_DURATION_TEMPORAL_SUMMARY.csv"
  )
)

summary_lines <- c(
  "TBI-TRACT EXACT LEAN DURATION TEST v2 COMPLETE",
  "",
...
writeLines(
  ...
  file.path(
    out_dir,
    "EXACT_LEAN_DURATION_SUMMARY.txt"
  )
)
